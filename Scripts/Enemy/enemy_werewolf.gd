...
# Animation
@onready var sprite: AnimatedSprite2D = $WerewolfSprite
...
func update_velocity() -> void:
	match current_state:
		...
		State.ATTACK:
			velocity = Vector2.ZERO
		State.PHASE:
			#If the werewolf is already following player, it will continue, otherwise it will not change velocity
			velocity = Vector2.ZERO
			if target:
				if sprite.frame == 8:
					var direction = target.global_position - global_position
...
func check_phase() -> void:
	var phase_space = $PhaseZone.get_overlapping_bodies()
	var phase_target = phase_space.filter(func(b):return b is Player)
	if phase_target.is_empty():
...
func attack(body) -> void:
	if body.is_in_group("player"):
		...
		$AttackArea/AttackEffect.play("attackeffect")
		current_state = State.ATTACK
