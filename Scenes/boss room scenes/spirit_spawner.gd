...
func spawn_spirits():
	...
	var selected_spirits = shuffled_spirits.slice(0, spawn_points.size())

	for i in range(spawn_points.size()):
		var spawn_point = spawn_points[i]
		var spirit_scene = selected_spirits[i % selected_spirits.size()]
		var spirit = spirit_scene.instantiate()

		# Place relative to room
		spirit.position = room.to_local(spawn_point.global_position)
		spirit.scale = Vector2(0.25, 0.25)
		spirit.z_index = 999

		room.add_child(spirit)
		print("[Spirits] Spawned %s at %s" % [spirit_scene.resource_path, spirit.position])
